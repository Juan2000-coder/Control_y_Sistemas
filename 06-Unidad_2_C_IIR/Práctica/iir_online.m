...
Hd = iir_elliptic_200_800;

[B, A] = tf(Hd);                 % Coeficientes del numerador (B) y denominador (A)
iir_output_m = filter(B, A, adc_signal);  % Aplicación del filtro
...
plot(f2, maf_f, '--b')
plot(f2, maf_m, '--r')
legend('INPUT SIGNAL' , 'IIR OUTPUT C', 'IIR OUTPUT MATLAB')
grid on
